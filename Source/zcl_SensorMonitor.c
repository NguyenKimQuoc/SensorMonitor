...
//  bdb_StartCommissioning(BDB_COMMISSIONING_MODES);
  ...
  osal_start_timerEx(zclSensorMonitor_TaskID, SENSORMONITOR_FIRST_REPORT_EVT, 5000);
}
...
//      LREP("p0state=0x%X\r\n", p0LastState);
  ...
  if ( events & SENSORMONITOR_OLED_OFF_EVT )
  {
    ////UART_String("Oled2");
    ssd1306_SetDisplayOn(0);
    return ( events ^ SENSORMONITOR_OLED_OFF_EVT );
  }
  
  if ( events & SENSORMONITOR_FIRST_REPORT_EVT )
  {
    ...
    return ( events ^ SENSORMONITOR_FIRST_REPORT_EVT );
  }
    ...
    if(keyCode & BV(7))
    {
      ...
      if (portAndAction & HAL_KEY_RELEASE) 
      {
        if(ssd1306_GetDisplayOn() == 1){
            ////UART_String("OFF");
          osal_start_timerEx(zclSensorMonitor_TaskID, SENSORMONITOR_OLED_OFF_EVT, 1000);
        }
        isPressFeatureButton = false;
//        osal_stop_timerEx(zclSensorMonitor_TaskID, SENSORMONITOR_OLED_OFF_EVT);
//        osal_start_timerEx(zclSensorMonitor_TaskID, SENSORMONITOR_OLED_OFF_EVT, 3000);
//        LREPMaster("Key1\r\n");
        //UART_String("Key1");
      }
...
//        LREPMaster("Key2\r\n");
        //UART_String("Key2");
        isPressFeatureButton = true;
        if(P0_7 == 0){         
          ssd1306_SetDisplayOn(1);
          ssd1306_DisplaySensor(TemperatureValue, HumidityValue, getConnected(), getBattPercentValue());
        }
...
//        osal_start_timerEx(zclSensorMonitor_TaskID, SENSORMONITOR_REPORT_EVT, 200);
    ...
    {
//      //LREPMaster("PIR\r\n");
      if(!isPressFeatureButton){
        if (portAndAction & HAL_KEY_RELEASE) 
        {
  //        osal_stop_timerEx(zclSensorMonitor_TaskID, SENSORMONITOR_OLED_DISPLAY_EVT);
          if(ssd1306_GetDisplayOn() == 1){
            ////UART_String("OFF");
            osal_start_timerEx(zclSensorMonitor_TaskID, SENSORMONITOR_OLED_OFF_EVT, 1000);
          }
